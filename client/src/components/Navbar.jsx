                        ...
                        <div className="ml-10 flex items-baseline space-x-4">
                            ...
                            {currentUser ? (
                                <Link to="/admin" className="ml-2 relative inline-flex items-center justify-center p-0.5 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-green-400 to-blue-600 group-hover:from-green-400 group-hover:to-blue-600 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800">
                                    <span className="relative px-5 py-2 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0 text-white flex items-center gap-2">
            ...
            {isOpen && (
                <div className="md:hidden bg-dark-bg/95 backdrop-blur-xl light:bg-white/95 light:text-gray-900 shadow-xl transition-colors duration-300">
                    <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                        ...
                        {currentUser ? (
                            <Link to="/admin" className="block text-accent hover:text-white px-3 py-2 rounded-md text-base font-medium font-bold flex items-center gap-2">
                                <FaUserCircle /> Dashboard
                            </Link>
                        ) : (
                            <Link to="/login" className="block text-accent hover:text-white px-3 py-2 rounded-md text-base font-medium font-bold">Admin Login</Link>
                        )}
                    </div>
                </div>
            )}
