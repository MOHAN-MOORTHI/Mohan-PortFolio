...
const AboutEditor = () => {
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            ...
            if (res.data.imagePath) {
                setAboutData({ ...aboutData, image: res.data.imagePath });
            }
        } catch (error) {
    ...
    const handleUpdateAbout = async (e) => {
        e.preventDefault();
        try {
            // Only send About specific fields, exclude socialLinks
            const { image, description1, description2 } = aboutData;
            await adminService.updateAboutFn({ image, description1, description2 });
